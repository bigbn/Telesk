...
!define PRODUCT_NAME "Phonty"
!define PRODUCT_VERSION "0.4.0"
!define PRODUCT_PUBLISHER "Phontycom LTD"
!define PRODUCT_WEB_SITE "http://www.phonty.com"
!define PRODUCT_DIR_REGKEY "Software\Microsoft\Windows\CurrentVersion\App Paths\phonty.exe"
...
; Finish page
!define MUI_FINISHPAGE_RUN "$INSTDIR\phonty.exe"
...
OutFile "..\phonty_setup.exe"
InstallDir "$PROGRAMFILES\Phonty"
...
Section "softphone" SEC01
  ...
  File "dist\sip.pyd"
  File "dist\phonty.exe"
  CreateDirectory "$SMPROGRAMS\Phonty"
  CreateShortCut "$SMPROGRAMS\Phonty\Phonty.lnk" "$INSTDIR\phonty.exe"
  CreateShortCut "$DESKTOP\Phonty.lnk" "$INSTDIR\phonty.exe"
...
Section -AdditionalIcons
  ...
  CreateShortCut "$SMPROGRAMS\Phonty\Website.lnk" "$INSTDIR\${PRODUCT_NAME}.url"
  CreateShortCut "$SMPROGRAMS\Phonty\Удалить.lnk" "$INSTDIR\uninst.exe"
SectionEnd

Section -Post
  WriteUninstaller "$INSTDIR\uninst.exe"
  WriteRegStr HKLM "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\phonty.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayName" "$(^Name)"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "UninstallString" "$INSTDIR\uninst.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\phonty.exe"
...
Section Uninstall
  ...
  Delete "$SMPROGRAMS\Phonty\Uninstall.lnk"
  Delete "$SMPROGRAMS\Phonty\Website.lnk"
  Delete "$DESKTOP\Phonty.lnk"
  Delete "$SMPROGRAMS\Phonty\Phonty.lnk"

  RMDir "$SMPROGRAMS\Phonty"
  RMDir "$INSTDIR"
